...
int BinarySearchTreeX<Comparable>::sum(BinaryNodeX<Comparable> *t) const
{
  if (t-> left == NULL && t->right == NULL)
    return t->element;
  if (t->left == NULL)
    return t->element + sum(t->right);
  if (t->right == NULL)
    return t->element + sum(t->left);
...
{
  return find(possibleAncestor, root) != NULL?
  find(x, find(possibleAncestor, root)) != NULL: false;
}
...
int BinarySearchTreeX<Comparable>::highestFull(BinaryNodeX<Comparable> *t) const
{
  if (t->left == NULL && t->right == NULL)
    return 1;
  if (t->left == NULL)
    return highestFull(t->right) + 1;
  if (t->right == NULL)
    return highestFull(t->left) + 1;
  int l = highestFull(t->left);
  int r = highestFull(t->right);
  return l > r ? l : r;
    //return 0;  // to be written by student
}
...
int BinarySearchTreeX<Comparable>::deepestFull(BinaryNodeX<Comparable> *t) const
{
  ...
  if (t->left == NULL || t->right == NULL)
    return 0;
  int l = deepestFull(t->left);
  int r = deepestFull(t->right);
  return l > r ? r + 1 : l + 1;
    //  return 0;  // to be written by student
}
